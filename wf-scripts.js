...
// ---------------------------------------
(function() {
    // Function to remove 'is-fill' class from a specified element
    function removeIsFill(element) {
        if (element) {
            element.classList.remove('is-fill');
        }
    }

    // Function to add 'is-fill' class to a specified element
    function addIsFill(element) {
        if (element) {
    ...
    if (insuranceSelect) {
        insuranceSelect.addEventListener("change", function () {
            ...
            if (selection === 'Yes, primary only') {
                // Remove 'is-fill' from #type
                removeIsFill(type);

                // Remove 'is-fill' from #type2 if it exists
                if (type2) {
                    removeIsFill(type2);
                }

                // Optionally, add 'is-fill' to #insurance2 if it exists
                if (insurance2) {
                    addIsFill(insurance2);
                }
            } else if (selection === 'Yes, primary & secondary') {
                // Remove 'is-fill' from both #type and #type2
                removeIsFill(type);
                removeIsFill(type2);

                // Ensure 'is-fill' is added to both #insurance and #insurance2
                addIsFill(insurance);
                addIsFill(insurance2);
            } else {
                // If InsuranceSelect is set to another option, add 'is-fill' to all relevant fields
                addIsFill(type);
                addIsFill(type2);
                addIsFill(insurance);
                addIsFill(insurance2);
    ...
    // Event Listener for statePrimary change
    if (statePrimary) {
        statePrimary.addEventListener('change', function () {
            // Remove 'is-fill' from #type
            removeIsFill(type);
    ...
    // Event Listener for stateSecondary change
    if (stateSecondary) {
        stateSecondary.addEventListener('change', function () {
            // Remove 'is-fill' from #type2
            removeIsFill(type2);
        });
    }
    ...
    if (type) {
        type.addEventListener("change", function () {
            // Remove 'is-fill' from #insurance
            removeIsFill(insurance);
        });
    }

    // Event Listener for type2 change
    if (type2) {
        type2.addEventListener("change", function () {
            // Remove 'is-fill' from #insurance2
            removeIsFill(insurance2);
        });
    }

    // Initial Setup: Add 'is-fill' to type/type2 and insurance/insurance2 if InsuranceSelect is already selected
    function initialIsFillSetup() {
        const selection = insuranceSelect.value.trim();
    ...
    // Call the initial setup on script load
    initialIsFillSetup();
